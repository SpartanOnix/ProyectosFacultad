   ...
   (cond
     ...
     [(list? sexp) (let* ([x (car sexp)] [xs (cdr sexp)])
                             (case x
                               ...
                               [(with) (withS (for/list ([i (car xs)]) (binding (car i) (par-tp (caddr i)) (parse (last i)))) (parse (cadr xs)))]
                               [(with*) (withS* (for/list ([i (car xs)]) (binding (car i) (par-tp (caddr i)) (parse (last i)))) (parse (cadr xs)))]
                               [(fun) (funS (car xs) (par-tp (caddr xs)) (parse (last xs)))]
                               [else (parse-app sexp)]))]))
         ...
         [(and (= (length f) 2) (or (symbol? x) (list? x)) (list? (cadr f)))
           (let ([fun (parse (car f))] [arg (parse-lst (cadr f))])
              (type-case SCFWBAE fun
                [idS (i) (appS fun arg)]
                [funS (par rType bod) (if (= (length par) (length arg)) (appS fun arg) (error "Algo no salio bien"))]
                [else (list fun arg)]))]
         [else (parse-lst f)])))
...
;; Toma una lista de parejas de condiciones y genera la sintáxis abstracta de una condicional en CFWBAE
...
;; Parsea los tipos de las variables
;; par-tp: list -> rType
(define (par-tp lst)
   (if (symbol? lst)
       (cond
          [(symbol=? lst 'number) (numberT)]
          [(symbol=? lst 'boolean) (booleanT)]
          [(list? lst) (funT (map (lambda (x) (par-tp x)) (filter (lambda (x) (not (symbol=? x '->))) lst)))])
