...
public class NodoBusqueda implements Comparable<NodoBusqueda> {
    ...
    public LinkedList<NodoBusqueda> getSucesores(){
		...
		Estado est;
		LinkedList<NodoBusqueda> lista = new LinkedList<NodoBusqueda>();
		NodoBusqueda nodo;
		for(Movimiento movimiento : Movimiento.values()){
			est = estado.aplicaAccion(movimiento);
			if(est != null){ 
				nodo = new NodoBusqueda(est);
				nodo.gn = gn + movimiento.costo();
				nodo.padre = this;
				nodo.accionPadre = movimiento;
				lista.add(nodo);
			}
		}
        return lista;
